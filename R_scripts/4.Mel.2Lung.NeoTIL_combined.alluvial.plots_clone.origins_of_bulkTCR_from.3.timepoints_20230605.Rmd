---
title: "4Mel.2Lung.NeoTIL combined alluvial plot for clonetype origin from 3 timepoints of bulkTCR - 20230605"
author: "Quy Ai Ngo"
date: "05-Jun-2023"
...
sample.3f <- c("B0", "ACTP", "T0") # for patient1,7,2,14
patient1 <- "14MH"
patient2 <- "13P4"
patient14 <- "1E18"
patient7 <- "13LN" #patient7c (T0 ATATIL)

sample.4fT0 <- c("B0", "ACTP", "T0.A", "T0.B") # for patient3,5
patient3 <- "1EDA" 
patient5 <- "1HU3"
...
listDF.Mel <- list(B0.M, ACTP.M, T0.M)
names(listDF.Mel) <- sample.3f
...
rm(B0.M, T0.M, ACTP.M, B0.L, T0.L, ACTP.L); invisible(gc()); gc()
...
AlvMel <- addCloneOri.3f(AlvMel)
AlvLung <- addCloneOri.3f(AlvLung)

# Factorize Sample for all patients:
AlvMel$Sample <- factor(AlvMel$Sample, levels = sample.3f)
...
write.table(AlvMel, paste0("4MelNeoTIL_bulkTCR_frequency.status_of_clone.origin_from.3.timepoints_for_alluvial.plot_20230605.txt"),
            sep = "\t", append = F, quote = F, row.names = F, col.names = T)  
write.table(AlvLung, paste0("2LungNeoTIL_bulkTCR_frequency.status_of_clone.origin_from.3.timepoints_for_alluvial.plot_20230605.txt"),
            sep = "\t", append = F, quote = F, row.names = F, col.names = T)  
...
AlvMel <- addAbsentFreq(sample.3f, AlvMel)
...
AlvMel.pl <- plotAlv(AlvMel, "pooled 14MH, 13LN, 13P4 & 1E18") + 
...
AlvLung.pl <- plotAlv(AlvLung, "pooled 1EDA & 1HU3") + 
  #scale_x_discrete(labels = c("B0", "ACTP", "T0") +
...
pdf(paste0("new_4Mel.2Lung.NeoTIL", "_bulkTCR_combined.alluvial.plot_clone.origin_from.3timepoints_20230605.pdf"), width = 10, height = 7)
